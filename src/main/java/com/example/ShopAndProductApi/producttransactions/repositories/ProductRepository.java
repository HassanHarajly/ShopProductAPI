...
@Transactional
    @Query(value =
    "INSERT INTO products(shop_id,product_name,product_quantity,product_barcode, product_price,image, latitude,longitude) SELECT ?1,?2, ?3, ?4,?5,?6, SHOP_LATITUDE, SHOP_LONGITUDE FROM shops where Shops.id = ?1",
    nativeQuery = true)
    void insertNewProduct(int shop_id,String product_name,int product_quantity,String product_barcode,double product_price, String image);
    @Query(
            value =
